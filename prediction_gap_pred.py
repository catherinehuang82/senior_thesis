...
#         print(df_attr.shape)
        ...
        #     print(inp.size())
            ...
            original_pred = torch.argmax(original_output, axis=-1).item()
#             orig_probs.append(torch.max(original_output_proba))
            ...
            #         print(f'noisy_channel: {noisy_channel}')
                ...
                noisy_pred = torch.argmax(noisy_output, axis=-1).item()
                noisy_output_proba = F.softmax(noisy_output, dim=1)
                if original_pred != noisy_pred:
                    # an m x n length list
                    all_pred_gaps = np.append(all_pred_gaps, 1)  # append 1 if they are not equal
                else:
                    all_pred_gaps = np.append(all_pred_gaps, 0)
#                 print(all_pred_gaps)
                del df_channel
    ...
    fid_dir = f'perturbation_gap_pred_test/{args.data}_sigma={args.sigma}_k={args.k}_m={args.m}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sigma', type=float, default=0.25) # 0.25, 0.5
